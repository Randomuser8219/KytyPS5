...
uint32_t LoopFuelExhausted(EmitterState& state) {
	...
	state.builder.AddFunction(spv::OpLoad, TypeU32(state), value, state.loop_fuel_variable);
	const auto spent = state.builder.AllocateId();
	state.builder.AddFunction(spv::OpUGreaterThan, TypeBool(state), spent, value,
...
void SpendLoopFuel(EmitterState& state) {
	const auto value = state.builder.AllocateId();
	state.builder.AddFunction(spv::OpLoad, TypeU32(state), value, state.loop_fuel_variable);
	const auto next = EmitBinaryU32(state, spv::OpIAdd, value, ConstantU32(state, 1));
	state.builder.AddFunction(spv::OpStore, state.loop_fuel_variable, next);
}
		...
		case CFG::TerminatorKind::ConditionalBranch: {
			...
			if (ctx.state.loop_fuel_variable != 0) {
				...
				if (true_exits != false_exits) {
					...
					if (true_exits) {
						ctx.state.builder.AddFunction(spv::OpLogicalOr, TypeBool(ctx.state), forced, condition, spent);
					} else {
						const auto alive = ctx.state.builder.AllocateId();
						ctx.state.builder.AddFunction(spv::OpLogicalNot, TypeBool(ctx.state), alive, spent);
						ctx.state.builder.AddFunction(spv::OpLogicalAnd, TypeBool(ctx.state), forced, condition, alive);
					}
					condition = forced;
				}
			}
...
void EmitProgram(EmitterState& state) {
	...
	if (state.loop_fuel_variable != 0) {
		state.builder.AddFunction(spv::OpVariable,
		                           TypePointer(state, spv::StorageClassFunction, TypeU32(state)),
		                           state.loop_fuel_variable, spv::StorageClassFunction);
	}
	for (uint32_t half = 0; half < state.lane_count; half++) {
